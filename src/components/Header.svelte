...
<header
  class={"z-[99] sticky top-0 duration-200 px-6 flex items-center justify-between border border-solid select-none " +
    (0 > y
...
>
  <h1 class="font-medium">
    <a href="/"
      ><b class="font-bold poppins theme-400">CutYourURL</b> URL Shortener</a
    >
  </h1>
  <div class="sm:flex items-center ml-auto pr-4 gap-4 hidden">
    {#each tabs as tab, index}
      <a href={tab.link} class="duration-100 hover:theme-400" title={tab.name}>
        <p>{tab.name}</p>
      </a>
    {/each}
    <a
